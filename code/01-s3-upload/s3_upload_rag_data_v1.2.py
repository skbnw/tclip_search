...
def should_upload_file(file_path: str, s3_key: str) -> Tuple[bool, str]:
    ...
    try:
        ...
        # タイムゾーンを考慮して比較（S3はUTC、ファイルシステムはローカル時刻）
        # 比較の際は、ファイルの更新日時がS3より新しい場合のみアップロード
        if file_datetime > s3_last_modified.replace(tzinfo=None):
            return (True, f"ファイルが更新されています (NAS: {file_datetime}, S3: {s3_last_modified})")
        else:
