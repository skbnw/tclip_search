...
# タイトル（ロゴ画像で表示、レスポンシブ対応）
import pathlib
script_dir = pathlib.Path(__file__).parent.absolute()
project_root = script_dir.parent.parent  # プロジェクトルート

logo_paths = [
    str(project_root / "image" / "longTclip-imade-long-2025-11-12.png"),  # 新しいロゴファイル
    str(project_root / "image" / "tclipLOGO.png"),  # ユーザー指定のパス（絶対パス）
    "image/longTclip-imade-long-2025-11-12.png",  # 相対パス（プロジェクトルートから）
...
if logo_found:
    ...
    try:
        import base64
        with open(logo_used_path, "rb") as f:
            logo_bytes = f.read()
            logo_base64 = base64.b64encode(logo_bytes).decode()
            
            # ファイル拡張子を取得してMIMEタイプを決定
            ext = os.path.splitext(logo_used_path)[1].lower()
            mime_types = {
                '.png': 'image/png',
                '.jpg': 'image/jpeg',
                '.jpeg': 'image/jpeg',
            ...
            <div class="logo-container">
                <img src="data:{mime_type};base64,{logo_base64}" class="logo-image" alt="番組データ検索β" />
            </div>
...
# 現在時刻をタブラインの右寄せで表示
now = get_jst_now()
weekday_en = now.strftime("%A")  # 英語の曜日
time_display = f"{now.strftime('%Y-%m-%d %H:%M')} {weekday_en}"
col_tabs, col_time = st.columns([8, 2])
with col_time:
    st.markdown(f"<div style='text-align: right; color: #666; font-size: 0.9em;'>{time_display}</div>", unsafe_allow_html=True)
...
with tab_latest:
    try:
        ...
        if latest_programs and len(latest_programs) > 0:
            ...
            # 全クリアボタンを右寄せで表示（最新データの下）
            col_clear_left, col_clear_right = st.columns([7, 3])
            with col_clear_right:
                if st.button("🔄 全てクリア", use_container_width=True, key="clear_all_button_latest"):
                    # 検索条件をクリア
                    st.session_state.search_channel = "すべて"
                    st.session_state.search_date = None
                    st.session_state.search_time = None
